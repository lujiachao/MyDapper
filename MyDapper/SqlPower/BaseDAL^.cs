...
namespace MyDapper.SqlPower
{
    public partial class BaseDAL<T> where T : class
    {
        ...
        /// </summary>
        /// <returns></returns>
        public virtual async Task<IEnumerable<T>> GetAllAsync(IDbConnection dbConnection)
        {
            return await dbConnection.GetAllAsync<T>();
        }
        ...
        /// <summary>
        /// 查询数据库,返回指定ID的对象,多用于事务
        /// </summary>
        /// <param name="id">主键的值</param>
        /// <returns></returns>
        public virtual async Task<T> FindByIDAsync(IDbConnection dbConnection,object id)
        {
            return await dbConnection.GetAsync<T>(id);    
        }
        ...
        /// <param name="info">指定的对象</param>
        /// <returns></returns>
        public virtual async Task<bool> InsertAsync(IDbConnection dbConnection,T info)
        {
            bool result = false;
            result = await dbConnection.InsertAsync(info) > 0;
            return await Task.FromResult(result);
        }
        ...
        /// <summary>
        /// 插入指定对象到数据库中,返回插入数据id,重载了insertasync方法,可以优化的方法,多用于事务。
        /// </summary>
        /// <param name="info">指定的对象</param>
        /// <returns name="id">返回插入数据的id</returns>
        public virtual async Task<int> InsertAsync(IDbConnection dbConnection,T info, string id)
        {
            return await dbConnection.InsertAsync(info);
        }
        ...
        /// <param name="list">指定的对象集合</param>
        /// <returns></returns>
        public virtual async Task<bool> InsertAsync(IDbConnection dbConnection,IEnumerable<T> list)
        {
                return await dbConnection.InsertAsync(list) > 0;
        }
        ...
        /// <param name="list">指定的对象集合</param>
        /// <returns></returns>
        public virtual async Task<bool> UpdateAsync(IDbConnection dbConnection,IEnumerable<T> list)
        {
        ...
        /// <param name="info">指定的对象</param>
        /// <returns></returns>
        public virtual async Task<bool> DeleteAsync(IDbConnection dbConnection,T info)
        {
            return await dbConnection.DeleteAsync(info);    
        }
        ...
        /// <param name="list">指定的对象集合</param>
        /// <returns></returns>
        public virtual async Task<bool> DeleteAsync(IDbConnection dbConnection,IEnumerable<T> list)
        {
            return await dbConnection.DeleteAsync(list);    
        }
        ...
        /// <summary>
        /// 根据指定对象的ID,从数据库中删除指定对象,多用于事务
        /// </summary>
        /// <param name="id">对象的ID</param>
        /// <returns></returns>
        public virtual async Task<bool> DeleteAsync(IDbConnection dbConnection,object id)
        {
            string query = string.Format("DELETE FROM {0} WHERE id = @id", TableName);
            var parameters = new DynamicParameters();    
            parameters.Add("@id", id);    
        ...
        /// </summary>
        /// <returns></returns>
        public virtual async Task<bool> DeleteAllAsync(IDbConnection dbConnection)
        {
            return await dbConnection.DeleteAllAsync<T>();
        }
